package lesson04.task1;

import java.io.File;
import java.io.FileNotFoundException;
import java.util.ArrayList;
import java.util.Collection;
import java.util.Scanner;

public class ProductService implements Readable{
    private ArrayList<String> res;

    public void process1() {

    };

    public void process2() {
        res = null;
    };

    /**
     * Сситывание и обработка данных
     * @param file файл
     * @return коллекция данных
     * @throws RuntimeException Исключение при обработке файла
     */
    public Collection<String> readTextFile(File file) {
        // ПРЕДУСЛОВИЯ
        if (file.exists()) {
            if (file.length() > 5000) {
                throw new RuntimeException("Размер файла больше 5мб. Чтение файла запрещено.");
            }
        } else {
            throw new RuntimeException("Файл не найден");
        }
        ArrayList<String> res = new ArrayList<>();
        try {
            Scanner scanner = new Scanner(file);
            while (scanner.hasNextLine()) {
                res.add(scanner.nextLine());
            }
            scanner.close();
        }catch (FileNotFoundException e) {
            e.printStackTrace();
        };

//        res = new ArrayList<>();

        process1();
        // ИНВАРИАНТ
        validateResult(res);

        process2();
        // ИНВАРИАНТ
        validateResult(res);

        // ПОСТУСЛОВИЕ
        if (res == null) {
            throw new RuntimeException("Ошибка обработки данных");
        }
        return res;
    };

    public void printText(ArrayList<String> res) {
        for (String str: res) {
            System.out.println(str);
        }
    }

    public void validateResult(Collection<String> res) {
        if (res == null || res.size() == 0) {
            throw new RuntimeException("Некорректное состояние объекта");
        }
    }
}


/**
 * Позволяет считывать данные
 */
interface Readable {
    /**
     * Сситывание и обработка данных
     * @param file файл
     * @return коллекция данных
     * @throws RuntimeException Исключение при обработке файла
     */
    Collection<String> readTextFile(File file) throws RuntimeException;
}